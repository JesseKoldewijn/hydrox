...
export const organizationSchema = versionedEntitySchema
...
export const projectSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
    ...
    key: z
      .string()
      .regex(/^[A-Z][A-Z0-9]+$/, "Project key must be uppercase like PROJ"),
    description: z.string().nullable(),
    issueCounter: z.number().int().nonnegative(),
  });

export const initiativeSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
    organizationId: entityIdSchema,
...
export const epicSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
...
export const issueSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
    projectId: entityIdSchema,
...
export const issueLinkSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
...
export const commentSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
...
export const sprintSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
    projectId: entityIdSchema,
    name: z.string(),
...
export const workflowStatusSchema = versionedEntitySchema
  .merge(softDeleteFieldsSchema)
  .extend({
    projectId: entityIdSchema,
    name: z.string(),
